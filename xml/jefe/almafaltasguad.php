		...
		<div class="col-sm-8 col-sm-offset-2">
			...
			<div class="well">
				...
				if($archivo1 and $archivo2){
					// Variables iniciales necesarias para la importación
					// numero de columnas que debe de tener la tabla de RegAlumn.txt
					$n_col_tabla=39;
					// posición donde se encuentra el apellido1 y apellido2
					$posapellido1 = 16;
					$posapellido2 = $posapellido1 + 1;
					// posición donde se encuentra el apellido1 y apellido2 del tutor
					$postutorapellido1 = 20;
					$postutorapellido2 = $postutorapellido1 + 1;
					...
					$alumnos = "CREATE TABLE  `alma` (
					 ...
					 `PRIMERTUTOR` VARCHAR ( 78 ) default NULL,
					 ...
					 constraint calma1 PRIMARY KEY(`CLAVEAL`)			 
...
/*					$crear = "ALTER TABLE  alma
			...
			";
					...
					mysqli_query($db_con, "ALTER TABLE  `alma` ADD INDEX (  `NOMBRE` )");
					mysqli_query($db_con, "ALTER TABLE  `alma` ADD INDEX (  `APELLIDOS` )");
					mysqli_query($db_con, "ALTER TABLE  `alma` ADD ADD  nc DOUBLE NULL AFTER `CLAVEAL`");
			
					// Apellidos unidos formando un solo campo.
					$SQL2 = "SELECT apellido1, apellido2, CLAVEAL, NOMBRE, PRIMERAPELLIDOTUTOR, SEGUNDOAPELLIDOTUTOR, NOMBRETUTOR FROM  alma";
					...
					while  ($row2 = mysqli_fetch_array($result2))
					{
						...
						$nombre = $row2[3];
						$nombre1 = trim($nombre);
						$apellidosP = trim($row2[6]). " " . trim($row2[4]). " " . trim($row2[5]);
						$apellidos1P = trim($apellidosP);
						
						$actualiza1= "UPDATE alma SET APELLIDOS = \"". $apellidos1 . "\", NOMBRE = \"". $nombre1 . "\", PADRE = \"". $apellidos1P . "\" where CLAVEAL = \"". $row2[2] . "\"";
...
/*					// Apellidos y nombre del padre.
			...
			<div align="center">
			  <input type="button" value="Volver atrás" name="boton" onClick="history.back(2)" class="btn btn-inverse" />
					...
					// Rellenamos datos en alma desde almafaltas
