...
for ($i = $today; $i <= ($today + 6); $i++) {
  ...
  $dayname = date("l",mktime(1,1,1,$current_month,$current_day,$current_year));
   ...
   $event_event7 = stripslashes($row["hora7"]);
	...
	echo '<a href="//'.$config['dominio'].'/'.$config['path'].'/reservas/reservar/index.php?year='.$current_year.'&today='.$current_day.'&month='.$current_month.'&servicio='.$servicio.'">';
...
echo '<br>';
echo '<a class="btn btn-primary btn-block" href="//'.$config['dominio'].'/'.$config['path'].'/reservas/reservar/index.php?servicio='.$servicio.'">Reservar...</a>';
echo '</div>';
echo '</div>';
